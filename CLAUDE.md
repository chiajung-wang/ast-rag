...
**Eval**: two sets. Dev = 50 questions in `evals/questions.jsonl` (prompt was tuned against the original 34). Held-out = 17 in `evals/questions-test.jsonl`, written from source after the prompt froze at `0061b3b`, never used for tuning. Both cover 7 tiers (recall / behavior / hard / definition / usage / cross-file / negative). Hybrid scoring: auto file-path check + LLM-as-judge (Claude Sonnet 4.6). N-run per question (default n=3); reports median score + variance. Results written to `evals/results/results-<mmdd-hhmm>-<agent>-<judge>.md`. Baseline: **94.0/95 (98.9%)** — dev set (50 q), n=3, `anthropic/claude-sonnet-5` agent+judge via OpenRouter, clean re-run 2026-08-13 after fixing the two bugs an earlier 91.0/95 run (2026-08-12) surfaced (one crash-handling fix, one eval-criteria fix — see `docs/plans/open-questions/milestone-6.md` A1). Replaces the old 63/67 (94%, n=1, haiku, 34 questions, direct Anthropic) — different provider, model, and question count, so not a fair prior comparison either. Only remaining miss is q20, and it is a rubric defect, not a model miss: criterion 3 expects the sync `CallbackManager` to respect `run_inline`, but sync `handle_event` (`callbacks/manager.py:255-336`) never reads that flag — only the async path does. Judge failed all 3 runs, variance 0.00. Left unfixed on purpose (fixing it moves dev to 95/95 and needs a re-run). See README "What the eval does not prove". Held-out end-to-end: 32/32 (100%), n=1, same models, run 2026-08-12.
