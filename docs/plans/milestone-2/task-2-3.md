...
**Acceptance criteria:**
- [ ] `find_symbol("RunnableSequence")` returns the class chunk (case-insensitive)
- [ ] `find_symbol("doesnotexist")` returns `None`
- [ ] `read_file("runnables/base.py", 1, 10)` returns 10 lines
- [ ] `read_file("runnables/base.py", 1, 500)` returns exactly 100 lines + truncation notice
- [ ] `read_file("nonexistent.py", 1, 5)` returns `[error: ...]` string (no exception)
- [ ] `search_corpus("RunnableSequence")` returns ≥1 result without hitting real OpenAI
- [ ] `retrieve("RunnableSequence", k=5)` puts the `find_symbol` hit first
- [ ] All tests pass
...
- [ ] **Step 3: Write `retrieval/pipeline.py`**
