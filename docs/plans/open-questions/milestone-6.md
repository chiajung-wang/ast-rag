...
**Clean re-run, 2026-08-13, both fixes in place: 94.0 / 95 (98.9%), $9.57 ($8.35 agent + $1.22 judge).** This is the current headline number, recorded in `evals/baseline.json`. The one point off max is q20. **Corrected 2026-08-20:** this was first recorded here as flakiness, "median=1, judge fail on one of three runs". The results file says otherwise. The judge failed q20 on all 3 runs at variance 0.00, which is consistent failure, not variance. The cause is a fourth grading-criteria bug of the same class as q28/q29/q30 below. Criterion 3 expects the answer to say the synchronous `CallbackManager` respects each handler's `run_inline` flag. Sync `handle_event` (`callbacks/manager.py:255-336`) never reads that flag. Only the async path does (`_ahandle_event_for_handler:379`, `ahandle_event:437,451`, `AsyncCallbackManager:1854,1964`). The agent's answer is correct and the criterion is not. Left unfixed on purpose: correcting it moves dev to 95/95 and needs a full re-run to rebaseline (~$9.57). Documented in `README.md` under "What the eval does not prove". q28/q29/q30 all scored a clean 2/2 under the full n=3 methodology, not just the earlier n=1 spot-check. Two questions (q25, q27) showed run-to-run judge variance (2 of 3 passing) but still held median 2 — worth watching if a future run drops them.
